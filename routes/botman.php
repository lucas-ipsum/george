...
//Antowort
// Rufe den Datenbankcontroller für die Abfrage auf und gib Antwort zurück
...
elseif (strlen($veranstaltung_context) > 0 && strlen($veranstaltungsart_context) > 0) {
    // Rufe den Datenbankcontroller für die Abfrage auf und gib Antwort zurück
    $uhrzeit = DBController::getDBUhrzeit($veranstaltung_context, $veranstaltungsart_context);
    $datum = DBController::getDBDatum($veranstaltung_context, $veranstaltungsart_context);
    $bot->reply($veranstaltung_context.' '.'('.$veranstaltungsart_context.') findet am '.$datum.' um '.$uhrzeit.' statt');
}
elseif (strlen($veranstaltung_context) > 0 && strlen($veranstaltungsart) > 0) {
    // Rufe den Datenbankcontroller für die Abfrage auf und gib Antwort zurück
    $uhrzeit = DBController::getDBUhrzeit($veranstaltung_context, $veranstaltungsart);
    $datum = DBController::getDBDatum($veranstaltung_context, $veranstaltungsart);
    $bot->reply($veranstaltung_context.' '.'('.$veranstaltungsart.')  findet am '.$datum.' um '.$uhrzeit.' statt');
}
else{
//Antowort
      // Rufe den Datenbankcontroller für die Abfrage auf und gib Antwort zurück
      $uhrzeit = DBController::getDBUhrzeit($veranstaltung, $veranstaltungsart_context);
      $datum = DBController::getDBDatum($veranstaltung, $veranstaltungsart_context);
      $bot->reply($veranstaltung.' '.'('.$veranstaltungsart_context.') findet am '.$datum.' um '.$uhrzeit.' statt');  //Platzhalter für Raum abfragen, der aus DB geholt wird
}
})->middleware($dialogflow);
...
//Speichern
    ...
    elseif (strlen($veranstaltung) > 0) {
      // Rufe den Datenbankcontroller für die Abfrage auf und gib Antwort zurück
      $klTermin = DBController::getDBklTermin($veranstaltung);
      $bot->reply('Die Klausur in ' . $veranstaltung . ' ist am '.$klTermin.'.');         //Dieser Fall wird aufgerufen, wenn die Veranstaltung in der Anfrage mit eingegeben wurde
    }
    else {
      // Rufe den Datenbankcontroller für die Abfrage auf und gib Antwort zurück
      $klTermin = DBController::getDBklTermin($veranstaltung_context);
      $bot->reply('Die Klausur in ' . $veranstaltung_context . ' ist am '.$klTermin.'.'); //Dieser Fall wird aufgerufen, wenn die Veranstaltung aus dem Context geholt wird
    }
})->middleware($dialogflow);
//################################################################################################################################################
